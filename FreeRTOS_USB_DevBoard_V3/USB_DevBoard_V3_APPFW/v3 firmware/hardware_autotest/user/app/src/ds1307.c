...
static rtc_ds1307_datetime_t writein_datetime = {
    .year = 1,
    .month = 2,
    .date = 3,
    .day = 4,
...
int ds1307_set_rtc_data (char register_value, char data) {
    printf("ds1307_set_rtc_data A entered\r\n");
    I2C_start(I2C1, (DS1307_ADDRESS << 1), I2C_Direction_Transmitter);
    printf("ds1307_set_rtc_data B entered\r\n");
    I2C_write(I2C1, register_value);
    printf("ds1307_set_rtc_data B entered\r\n");
    I2C_write(I2C1, data);
    printf("ds1307_set_rtc_data B entered\r\n");
    I2C_stop(I2C1);
    printf("ds1307_set_rtc_data C entered\r\n");
...
int ds1307_init_rtc (int first_run_flag) {
    ...
    */
    ...
    if (ds1307_set_rtc_data(CONTROL_REGISTER, 0x10) == -1) {
        return RTC_DS1307_BAD;
    }
    ...
    if (first_run_flag) {
        return ds1307_set_rtc_datetime(&writein_datetime);
...
int8_t date_time_compare(rtc_ds1307_datetime_t* readout_datetime)
{
    int8_t ret = -1;
    
    if((readout_datetime->year == writein_datetime.year)&& \
        (readout_datetime->month == writein_datetime.month)&& \
        (readout_datetime->date == writein_datetime.date)&& \
        (readout_datetime->day == writein_datetime.day)&& \
        (readout_datetime->hour == writein_datetime.hour)&& \
        (readout_datetime->minutes == writein_datetime.minutes)&& \
        (readout_datetime->seconds == writein_datetime.seconds))
    {
        ret = 0;
    }
    else
