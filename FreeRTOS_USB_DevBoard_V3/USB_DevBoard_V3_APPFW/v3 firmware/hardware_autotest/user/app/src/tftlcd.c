...
static void LCD_Gpio_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);	 //使能PB,PG端口时钟
    ...
    */
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_6;	//PG6.7.8 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
	GPIO_Init(GPIOA, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB.5
	GPIO_SetBits(GPIOA,GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4);	 //PG6.7.8输出高
 ...
 * @return  void
 */
static void LCD_SPI_Send(u8 *data, uint32_t size)
{
...
{
    //LCD_Show_Image(x, y, 240, 240, gImage_1);
}


void Display_Company_LOGO(u16 x, u16 y, const u8 *p)
{
    LCD_Show_Image(x, y, 240, 240, p);
}
