...
static void DeviceDescriptor_Init(void)
{
    ...
    Device_Descriptor.idProduct = 0x0001;
...
void Descriptors_Init(void)
{
    ...
    Descriptor_Set.Configuration_Descriptor.wTotalLength = (uint16_t)(sizeof(Configuration_Descriptor_t) + sizeof(Interface_Descriptor_t) + sizeof(HID_Descriptor_t) + sizeof(Endpoint_Descriptor_t));
    ...
    Descriptor_Set.Interface_Descriptor.bNumEndpoints = 0x01;                               //非0端点的数目(USB鼠标只需要一个中断输入端点, 因此设置为1)
    ...
    Descriptor_Set.Interface_Descriptor.bInterfaceProtocol = 0x02;                          //如果子类为支持引导启动的子类，则协议可选择鼠标和键盘，键盘代码为0x01, 鼠标代码为0x02
    ...
    Descriptor_Set.HID_Descriptor.HID_SubDescriptor[0].wDescriptorLength = 50;

    Descriptor_Set.Endpoint_Descriptor.bLength = sizeof(Endpoint_Descriptor_t);
    Descriptor_Set.Endpoint_Descriptor.bDescriptorType = 0x05;
    Descriptor_Set.Endpoint_Descriptor.bEndpointAddress = 0x81;                             //使用输入端点1
    Descriptor_Set.Endpoint_Descriptor.bmAttributes = 0x03;                                 //D1~D0端点传输类型选择(该端点为中断端点)
    Descriptor_Set.Endpoint_Descriptor.wMaxPacketSize = 0x0010;                             //端点的最大包长(这里设置为16个字节)
    Descriptor_Set.Endpoint_Descriptor.bInterval = 0x0A;                                    //端点查询时间为10个帧间隔时间即10ms
